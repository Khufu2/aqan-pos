...
function KpiCard({ label, value, note, icon, tone = "blue" }: { label: string; value: string; note: string; icon: IconName; tone?: string }) {
  return <article className="kpi-card"><div className={`kpi-icon ${tone}`}><Icon name={icon} size={19}/></div><div className="kpi-heading"><span>{label}</span><button aria-label={`More options for ${label}`}><Icon name="more" size={17}/></button></div><strong className="kpi-value">{value}</strong><span className="kpi-note"><b>â</b> {note}</span></article>;
}
...
function Dashboard({ onNavigate, data, displayName, language }: { onNavigate: (view: string) => void; data: AqanData; displayName: string; language: AqanLanguage }) {
  ...
  return <div className="page-content">
    <section className="welcome-row">
      <div><div className="eyebrow"><span className="live-dot"/> {todayLabel(language)}</div><h1>{timeGreeting(language)}, {firstName(displayName)}.</h1><p>{language === "sw" ? "Hiki ndicho kinachoendelea kwenye biashara yako ya vifaa tiba leo." : "Hereâs what is moving across your medical supply business today."}</p></div>
...
function PointOfSale({ products, customers, vatRate, onToast, onCheckout }: {
  ...
  return <div className="pos-page">
    <section className="pos-products">
      <div className="pos-title"><div><span className="section-kicker">Sales desk</span><h1>Point of sale</h1><p>Select equipment and build the order.</p></div><span className="register-badge"><span/> Register 01 Â· Open</span></div>
      <div className="catalog-search"><Icon name="search" size={19}/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search products, SKU or scan barcode"/><kbd>â K</kbd></div>
...
function CustomersView({ customers, onToast, onAdd }: { customers: Customer[]; onToast: (message: string) => void; onAdd: () => void }) {
  ...
  return <div className="workspace-page"><WorkspaceHeader kicker="Relationships" title="Customers" description="Every hospital, clinic, contact, consent record and opportunity in one intelligent CRM." action="New customer" onAction={onAdd}/>
    ...
    <section className="table-panel"><div className="table-tools"><label><Icon name="search" size={17}/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search customers, contacts or city"/></label><div className="filter-row"><button className="active">All customers</button><button>Hospitals</button><button>Clinics</button><button>High value</button></div><button className="tool-button">Import</button></div>
      <div className="data-table customer-table"><div className="table-head"><span>Customer</span><span>Contact</span><span>Lifetime value</span><span>Last activity</span><span>AI fit</span><span/></div>{filtered.map((c) => <div className="table-row" key={c.name}><span className="customer-cell"><i>{c.initials}</i><span><b>{c.name}</b><small>{c.type} Â· {c.city}</small></span></span><span>{c.contact}</span><span><b>{c.spend}</b></span><span>{c.last}</span><span><em className="score"><i style={{width:`${c.score}%`}}/>{c.score}</em></span><button onClick={() => onToast(`Customer profile opened for ${c.name}.`)} aria-label={`Open ${c.name}`}><Icon name="arrow" size={17}/></button></div>)}</div>
    </section>
  </div>;
}
...
function ServiceView({ requests, onToast, onAdd }: { requests: AqanData["serviceRequests"]; onToast: (message: string) => void; onAdd: () => void }) {
  ...
  return <div className="workspace-page"><WorkspaceHeader kicker="After-sales care" title="Service & warranty" description="Protect every serialised device from installation through its full service life." action="Schedule visit" onAction={onAdd}/>
    ...
    <section className="service-layout"><article className="table-panel service-tickets"><div className="panel-heading"><div><span className="section-kicker">Service desk</span><h2>Active requests</h2></div><button>View history</button></div>{tickets.map((t) => <div className="ticket" key={t.id}><span className="ticket-icon"><Icon name="service" size={19}/></span><div><div><b>{t.id}</b><em className={`pill ${t.level.toLowerCase().replace(" ", "-")}`}>{t.level}</em></div><strong>{t.customer}</strong><p>{t.device}</p><small>{t.issue}</small></div><div className="ticket-due"><small>Visit due</small><b>{t.due}</b><button onClick={() => onToast(`${t.id} assigned to the service team.`)}>Manage <Icon name="arrow" size={14}/></button></div></div>)}</article>
      <aside className="panel warranty-panel"><div className="panel-heading"><div><span className="section-kicker">Upcoming</span><h2>Warranty expiries</h2></div></div><div><span className="date-box"><b>28</b><small>AUG</small></span><p><strong>3 Ã ECG Machine E6</strong><small>Tumaini Regional Clinic</small></p><em>4 days</em></div><div><span className="date-box"><b>02</b><small>SEP</small></span><p><strong>Patient Monitor X12</strong><small>Kairuki Hospital</small></p><em>9 days</em></div><div><span className="date-box"><b>08</b><small>SEP</small></span><p><strong>Oxygen Concentrator 10L</strong><small>Upendo Health Centre</small></p><em>15 days</em></div><button className="renew-button" onClick={() => onToast("Renewal messages prepared for 5 customers.")}><Icon name="campaign" size={16}/> Prepare renewal outreach</button></aside>
    </section>
  </div>;
}
...
function CampaignsView({ campaigns, onToast, onAdd }: { campaigns: AqanData["campaigns"]; onToast: (message: string) => void; onAdd: () => void }) {
  ...
  return <div className="workspace-page"><WorkspaceHeader kicker="Customer growth" title="Campaigns" description="Reach the right healthcare buyers when products arrive, warranties expire or demand changes." action="New campaign" onAction={onAdd}/>
    ...
    <section className="campaign-grid"><article className="panel campaign-list"><div className="panel-heading"><div><span className="section-kicker">Active & recent</span><h2>Campaign performance</h2></div><button>View all</button></div><div className="campaign-row"><span className="campaign-channel whatsapp">WA</span><div><strong>Monitor accessories restock</strong><p>42 WhatsApp recipients Â· Sent 22 Aug</p></div><span><b>76%</b><small>read</small></span><span><b>9</b><small>replies</small></span><em className="pill success">Active</em></div><div className="campaign-row"><span className="campaign-channel email">@</span><div><strong>Annual service renewal</strong><p>31 email recipients Â· Sent 18 Aug</p></div><span><b>61%</b><small>opened</small></span><span><b>6</b><small>renewed</small></span><em className="pill viewed">Completed</em></div><div className="campaign-row"><span className="campaign-channel mixed">â</span><div><strong>Oxygen concentrator offer</strong><p>68 multi-channel recipients Â· Sent 11 Aug</p></div><span><b>54%</b><small>engaged</small></span><span><b>4</b><small>sales</small></span><em className="pill viewed">Completed</em></div></article>
      <aside className="panel message-preview"><span className="section-kicker">Personalized preview</span><h2>How your customer sees it</h2><div className="phone-message"><span className="message-logo">A</span><p>Habari <b>Dr. Asha</b>,</p><p>Our new <b>Portable Ultrasound U8</b> has arrived. Based on your recent imaging enquiry, we reserved an early demonstration slot for Kijiji Clinic.</p><p className="message-cta">View product & request demo â</p><small>Reply STOP to opt out</small></div><button onClick={() => onToast(`${campaigns.length || 3} campaign records are available. Provider delivery activates when WhatsApp/email credentials are added.`)}>Delivery readiness</button></aside>
    </section>
  </div>;
}
...
function PersonalSettings({ session, language, theme, fontSize = "standard", onLanguageChange, onThemeChange, onFontSizeChange = () => undefined, onToast }: { session: Session; language: AqanLanguage; theme: AqanTheme; fontSize?: AqanFontSize; onLanguageChange: (language: AqanLanguage) => void; onThemeChange: (theme: AqanTheme) => void; onFontSizeChange?: (fontSize: AqanFontSize) => void; onToast: (message: string) => void }) {
  ...
  return <div className="workspace-page"><WorkspaceHeader kicker="Personal workspace" title="My account & preferences" description="Choose your display language, text size, appearance, account identity and sign-in security." action="Save profile" onAction={() => document.getElementById("aqan-personal-settings")?.requestSubmit()}/><section className="settings-grid"><form id="aqan-personal-settings" className="table-panel" onSubmit={saveProfile}><div className="panel-heading"><div><span className="section-kicker">Identity</span><h2>Profile</h2></div><span className="pill success">Secure account</span></div><div className="form-grid" style={{padding:20}}><label>Full name<input name="full_name" required defaultValue={String(profile.full_name || "")}/></label><label>Work email<input value={session.user.email || ""} readOnly aria-readonly="true"/></label><label className="span-two">Phone number<input name="phone" type="tel" defaultValue={String(profile.phone || "")} placeholder="+255 â¦"/></label><label className="span-two">Display language<select value={language} onChange={(event) => onLanguageChange(event.target.value as AqanLanguage)}><option value="en">ð¬ð§ English</option><option value="sw">ð¹ð¿ Kiswahili</option></select></label><div className="span-two"><span className="settings-label">Text size</span><p className="settings-help">AQAN now uses a more readable default. Choose the size that is comfortable for you.</p><div className="font-size-options">{(["small", "standard", "large"] as AqanFontSize[]).map((item) => <button key={item} type="button" className={fontSize === item ? "selected" : ""} onClick={() => onFontSizeChange(item)} aria-pressed={fontSize === item}><b className={`font-preview ${item}`}>Aa</b><span>{item === "small" ? "Small" : item === "standard" ? "Standard" : "Large"}</span></button>)}</div></div><div className="span-two"><span className="settings-label">Appearance</span><div className="theme-options">{(["light", "dark", "system"] as AqanTheme[]).map((item) => <button key={item} type="button" className={theme === item ? "selected" : ""} onClick={() => onThemeChange(item)}><b>{item === "light" ? "âï¸" : item === "dark" ? "ð" : "â"}</b><span>{item === "light" ? "Light" : item === "dark" ? "Dark" : "System"}</span></button>)}</div></div></div><div className="modal-actions" style={{padding:"0 20px 20px"}}><button className="button primary" disabled={busy}>{busy ? "Savingâ¦" : "Save personal settings"}</button></div></form><section className="table-panel security-panel"><div className="panel-heading"><div><span className="section-kicker">Account security</span><h2>Update password</h2></div></div><p>Use a strong password unique to AQAN. You can change it at any time without losing your workspace access.</p><form className="form-grid" onSubmit={savePassword}><label className="span-two">New password<input name="password" type="password" minLength={8} required autoComplete="new-password" placeholder="At least 8 characters"/></label><div className="span-two modal-actions"><button className="button secondary" disabled={busy}>{busy ? "Updatingâ¦" : "Update password"}</button></div></form><div className="settings-security-note"><Icon name="shield" size={18}/><span>Permissions are enforced by AQANâs Supabase row-level security, not by the interface alone.</span></div></section></section>{error ? <div className="form-error">{error}</div> : null}</div>;
}

function SettingsView({ settings, membership, session, language, theme, fontSize = "standard", onLanguageChange, onThemeChange, onFontSizeChange = (nextFontSize) => { localStorage.setItem("aqan-font-size", nextFontSize); void updateMyProfile({ fullName: String(session.user.user_metadata?.full_name || session.user.email?.split("@")[0] || ""), phone: String(session.user.user_metadata?.phone || ""), language, theme, fontSize: nextFontSize }).catch(() => undefined); window.dispatchEvent(new CustomEvent("aqan-font-size-change", { detail: nextFontSize })); }, onToast, onRefresh }: { settings: BusinessSettings | null; membership: Membership; session: Session; language: AqanLanguage; theme: AqanTheme; fontSize?: AqanFontSize; onLanguageChange: (language: AqanLanguage) => void; onThemeChange: (theme: AqanTheme) => void; onFontSizeChange?: (fontSize: AqanFontSize) => void; onToast: (message: string) => void; onRefresh: () => Promise<void> }) {
  const [busy, setBusy] = useState(false); const canEdit = ["owner", "admin"].includes(membership.role);
  ...
  return <><PersonalSettings session={session} language={language} theme={theme} fontSize={fontSize} onLanguageChange={onLanguageChange} onThemeChange={onThemeChange} onFontSizeChange={onFontSizeChange} onToast={onToast}/><div className="workspace-page"><section className="settings-grid"><section className="table-panel"><div className="panel-heading"><div><span className="section-kicker">Template studio</span><h2>Document appearance</h2></div></div><form className="form-grid" style={{padding:20}} onSubmit={saveLayout}><label>Invoice & quotation layout<select name="layout" defaultValue={settings?.document_layout || "modern"} disabled={!canEdit}><option value="classic">Classic â formal healthcare supplier</option><option value="modern">Modern â spacious and brand-led</option><option value="compact">Compact â efficient A4 / thermal friendly</option></select></label><label>Brand accent<input name="accent" type="color" defaultValue={settings?.quotation_accent || "#14a7a0"} disabled={!canEdit}/></label><div className="span-two modal-actions"><button className="button primary" disabled={!canEdit || busy}>{busy ? "Savingâ¦" : "Apply document appearance"}</button></div></form></section><section className="table-panel settings-shortcuts"><span className="section-kicker">Business controls</span><h2>What owners can manage</h2><div><b>Company profile</b><span>Legal details, TIN, VRN and address</span></div><div><b>Commercial terms</b><span>Bank accounts, VAT, quotation & delivery conditions</span></div><div><b>Branding</b><span>Official logo and document styling</span></div></section></section></div><SettingsBrandingLegacy settings={settings} membership={membership} onToast={onToast} onRefresh={onRefresh}/></>;
}
...
function IntelligenceView({ data, membership, onToast, onRefresh }: { data: AqanData; membership: Membership; onToast: (message: string) => void; onRefresh: () => Promise<void> }) {
  ...
  return <div className="workspace-page intelligence-page"><WorkspaceHeader kicker="Business intelligence" title="Ask AQAN AI" description="Gemini analyses only the signed-in workspaceâs sales, stock, CRM, quotations and service data." action="Copy insight" onAction={() => void copyInsight()}/>
    <section className="ai-workspace"><div className="ai-chat"><div className="ai-chat-heading"><div className="ai-orb large"><Icon name="sparkles" size={24}/></div><div><span>AQAN AI Â· Gemini</span><h2>Your medical supply analyst</h2><p>Analyse the live workspace or ask AQAN to safely prepare an operational action.</p></div><em><span/> {asking ? "Analysing" : error ? "Needs attention" : "Ready"}</em></div><div className="answer-card"><span>{error ? "CONNECTION STATUS" : "CONVERSATION"}</span>{submittedQuestion ? <div className="ai-user-message"><small>You</small><p>{submittedQuestion}</p></div> : null}<div className={error ? "ai-assistant-message error" : "ai-assistant-message"}><small>AQAN AI</small><p>{asking ? "Analysing the secured AQAN workspaceâ¦" : error ? `I could not return a generated response: ${error}` : answer}</p></div><div className="answer-metrics"><div><small>Recorded sales value</small><strong>{formatTzs(revenue)}</strong><em>{data.sales.length} transactions</em></div><div><small>Strongest category</small><strong>{categoryName}</strong><em>by current stock value</em></div><div><small>CRM leads</small><strong>{data.crmLeads.length}</strong><em>available to analyse</em></div></div><small className="sources">Live scope: {data.products.length} products Â· {data.customers.length} customers Â· {data.crmLeads.length} CRM leads Â· {data.sales.length} recent sales</small></div><form className="ai-input" onSubmit={(e) => {e.preventDefault(); void ask(question);}}><Icon name="sparkles" size={18}/><input value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Try: Add a product, create a quotation, or create an invoiceâ¦"/><button type="submit" disabled={asking || !question.trim()}>{asking ? "Thinkingâ¦" : "Ask"} <Icon name="arrow" size={15}/></button></form>{action ? <AiActionPanel kind={action.kind} prompt={action.prompt} data={data} membership={membership} onDismiss={() => setAction(null)} onCreated={onRefresh} onToast={onToast}/> : null}</div>
...
function FloatingAqanChat({ data, membership, onToast, onRefresh, onOpenIntelligence }: { data: AqanData; membership: Membership; onToast: (message: string) => void; onRefresh: () => Promise<void>; onOpenIntelligence: () => void }) {
  ...
  const [asking, setAsking] = useState(false);
  const [error, setError] = useState("");
  const [messages, setMessages] = useState<AiChatMessage[]>([{ id: "welcome", role: "assistant", text: "Iâm AQAN AI. Ask me about live stock, customers, sales or CRMâor ask me to add a product, prepare a quotation or create an invoice." }]);
  const [action, setAction] = useState<{ kind: AiActionKind; prompt: string } | null>(null);
  const ask = async (value: string) => {
    ...
    if (intent) {
      ...
      setError("");
      setMessages((current) => [...current, { id: `${messageId}-assistant`, role: "assistant", text: intent === "product" ? "Iâve opened the secure product form. Add the catalogue facts and confirm when ready." : intent === "quotation" ? "Iâve opened the quotation workspace. Select the customer, live catalogue lines and validity date." : "Iâve opened the invoice workspace. Confirm the customer, stock lines and recorded payment method before issuing it." }]);
...
function CreateModal({ mode, customers, products, onClose, onSubmit }: {
  ...
  return <div className="modal-backdrop"><form className="create-modal" onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="create-title"><button type="button" className="modal-close" onClick={onClose} aria-label="Close"><Icon name="close"/></button><span className="section-kicker">{definition.kicker}</span><h2 id="create-title">{definition.title}</h2><p>Saved directly to the secured AQAN workspace.</p><div className="form-grid">
    ...
    {mode === "customer" ? <><label className="span-two">Facility name<input name="name" required placeholder="e.g. Mikocheni Specialist Hospital"/></label><label>Type<select name="customer_type"><option>Hospital</option><option>Clinic</option><option>Diagnostic centre</option><option>Pharmacy</option><option>NGO</option><option>Distributor</option></select></label><label>City<input name="city" required defaultValue="Dar es Salaam"/></label><label>Contact person<input name="contact_name" placeholder="Full name"/></label><label>Email<input name="email" type="email" placeholder="procurement@example.org"/></label><label className="span-two">Phone<input name="phone" type="tel" placeholder="+255 â¦"/></label></> : null}
    {mode === "quotation" ? <>{customerSelect}<label>Valid until<input name="valid_until" type="date" required defaultValue={defaultValidUntil}/></label><label className="span-two">Product / equipment<select name="product_id" required defaultValue=""><option value="" disabled>Select a stocked product</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name} Â· {formatTzs(product.price)}</option>)}</select></label><label>Quantity<input name="quantity" type="number" min="1" step="1" defaultValue="1" required/></label><label className="span-two">Notes<textarea name="notes" rows={4} placeholder="Scope, installation, warranty or delivery notes"/></label><div className="form-note span-two">The quotation includes the current commercial terms, payment terms and bank instructions from Business settings.</div></> : null}
...
export default function Home() {
  ...
  useEffect(() => { const sync = (event: Event) => { const nextFontSize = (event as CustomEvent<AqanFontSize>).detail; if (["small", "standard", "large"].includes(nextFontSize)) setFontSize(nextFontSize); }; window.addEventListener("aqan-font-size-change", sync); return () => window.removeEventListener("aqan-font-size-change", sync); }, []);
  useEffect(() => {
    if (!session) return;
    ...
    const nextFontSize = ["small", "standard", "large"].includes(String(metadata.aqan_font_size)) ? metadata.aqan_font_size as AqanFontSize : (localStorage.getItem("aqan-font-size") as AqanFontSize | null);
    const timer = window.setTimeout(() => { if (nextLanguage) setLanguage(nextLanguage); if (nextTheme) setTheme(nextTheme); if (nextFontSize) setFontSize(nextFontSize); }, 0);
  ...
  if (loading && session) return <main className="app-shell workspace-loading" data-theme={activeTheme} data-font-size={fontSize} aria-live="polite"><div className="workspace-loading-content"><span className="workspace-spinner" aria-hidden="true"/><div><span className="section-kicker">AQAN BIOMEDICAL POS</span><h2>Getting your workspace ready</h2><p>Securely checking your access and latest business dataâ¦</p></div></div></main>;
  ...
  return <main className="app-shell" data-theme={activeTheme} data-font-size={fontSize}>
