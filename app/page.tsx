...
const nav = [
  ...
  { id: "sell", label: "Point of sale", icon: "sale" as IconName },
  { id: "inventory", label: "Inventory", icon: "inventory" as IconName },
...
const swNav: Record<string, string> = { dashboard: "Muhtasari", sell: "Mauzo", inventory: "Bidhaa", customers: "Wateja", crm: "CRM ya vituo", quotes: "Nukuu", service: "Huduma", campaigns: "Kampeni", logistics: "Usafirishaji", insights: "Akili", team: "Wafanyakazi", settings: "Mipangilio" };
...
function PointOfSale({ products, customers, vatRate, onToast, onCheckout }: {
  ...
  onCheckout: (items: Array<{ product_id: string; quantity: number }>, paymentMethod: string, customerId: string | null, paymentProvider: string | null, contact: { name: string; phone: string; email: string }) => Promise<{ message: string; invoiceNumber: string; total: number } | null>;
}) {
  ...
  const [completedSale, setCompletedSale] = useState<{ invoiceNumber: string; total: number; paymentMethod: string } | null>(null);
  ...
  const complete = async (paymentMethod: string) => {
    setCompleting(true);
    try {
      const result = await onCheckout(cartRows.map((row) => ({ product_id: row.id, quantity: row.qty })), paymentMethod, customerId || null, paymentMethod === "mobile_money" ? paymentProvider : null, { name: contactName, phone: contactPhone, email: contactEmail });
      if (!result) return;
      setReceiptOpen(false);
      setCart({});
      setCompletedSale({ invoiceNumber: result.invoiceNumber, total: result.total, paymentMethod });
  ...
  return <div className="pos-page">
    ...
    {receiptOpen ? <div className="modal-backdrop"><div className="payment-modal" role="dialog" aria-modal="true" aria-labelledby="payment-title"><button className="modal-close" disabled={completing} onClick={() => setReceiptOpen(false)} aria-label="Close"><Icon name="close"/></button><span className="success-icon"><Icon name="check" size={30}/></span><span className="section-kicker">Secure payment</span><h2 id="payment-title">Collect {formatTzs(total)}</h2><p>Stock, customer, payment method and invoice are posted together. No external mobile-money confirmation is attempted.</p><div className="form-grid compact"><label>Buyer / facility<input value={contactName} onChange={(event) => setContactName(event.target.value)} placeholder="Walk-in buyer or facility"/></label><label>Mobile number<input value={contactPhone} onChange={(event) => setContactPhone(event.target.value)} placeholder="+255…"/></label><label className="span-two">Email<input value={contactEmail} onChange={(event) => setContactEmail(event.target.value)} placeholder="Optional email"/></label></div><div className="payment-methods complete"><button disabled={completing} onClick={() => void complete("cash")}>Cash <small>Record cash payment</small></button><button disabled={completing} onClick={() => void complete("card")}>Card <small>Visa or Mastercard</small></button><button disabled={completing} onClick={() => void complete("mobile_money")}>Mobile money <small>Record selected provider</small></button><button disabled={completing} onClick={() => void complete("bank_transfer")}>Bank transfer <small>Record account transfer</small></button><button disabled={completing} onClick={() => void complete("credit")}>Credit sale <small>Record customer credit</small></button></div><label className="mobile-provider">Mobile provider<select value={paymentProvider} onChange={(event) => setPaymentProvider(event.target.value)}><option value="m_pesa">M-Pesa</option><option value="airtel_money">Airtel Money</option><option value="tigo_pesa">Tigo Pesa</option><option value="halopesa">HaloPesa</option><option value="mixx_by_yas">Mixx by Yas</option><option value="other">Other</option></select></label>{completing ? <p className="form-note">Validating stock and securing the transaction…</p> : null}</div></div> : null}
    {completedSale ? <div className="modal-backdrop"><div className="payment-modal sale-complete" role="dialog" aria-modal="true" aria-labelledby="sale-complete-title"><span className="success-icon"><Icon name="check" size={30}/></span><span className="section-kicker">Sale completed</span><h2 id="sale-complete-title">{completedSale.invoiceNumber}</h2><p>{formatTzs(completedSale.total)} recorded via {completedSale.paymentMethod.replaceAll("_", " ")}. Stock and customer history are already updated.</p><div className="modal-actions"><button className="button secondary" onClick={() => { const popup = window.open("", "_blank", "width=600,height=760"); if (!popup) { onToast("Allow pop-ups to print the invoice receipt."); return; } popup.document.write(`<!doctype html><html><head><title>${safeDocumentText(completedSale.invoiceNumber)}</title><style>body{font-family:Arial;padding:38px;color:#16364d}h1{font-size:24px;border-bottom:3px solid #0ea5e9;padding-bottom:16px}.total{font-size:25px;font-weight:bold;margin:28px 0}.note{background:#f0f9fd;padding:15px;border-radius:10px}@media print{body{padding:10px}}</style></head><body><h1>AQAN Biomedical Solutions</h1><h2>Invoice ${safeDocumentText(completedSale.invoiceNumber)}</h2><div class="total">${safeDocumentText(formatTzs(completedSale.total))}</div><p>Payment method: ${safeDocumentText(completedSale.paymentMethod.replaceAll("_", " "))}</p><p class="note">This transaction is recorded in AQAN BIOMEDICAL POS. Open the sales workspace for the complete audit record.</p><script>window.onload=()=>window.print()</script></body></html>`); popup.document.close(); }}>Print receipt</button><button className="button primary" onClick={() => setCompletedSale(null)}>Start another sale</button></div></div></div> : null}
  </div>;
}
...
function FacilityCrmLegacy({ data, membership, onToast, onRefresh }: { data: AqanData; membership: Membership; onToast: (message: string) => void; onRefresh: () => Promise<void> }) {
  ...
  const reach = async (lead: AqanData["crmLeads"][number], channel: "email" | "whatsapp") => {
    ...
    try {
      ...
      if (channel === "email" && lead.email) window.location.href = `mailto:${lead.email}?subject=${encodeURIComponent("AQAN Biomedical equipment support")}&body=${encodeURIComponent(intro)}`;
      if (channel === "whatsapp" && whatsappUrl) {
        if (whatsappTab && !whatsappTab.closed) whatsappTab.location.href = whatsappUrl;
...
export default function Home() {
  ...
  const checkout = async (items: Array<{ product_id: string; quantity: number }>, paymentMethod: string, customerId: string | null, paymentProvider: string | null, contact: { name: string; phone: string; email: string }) => {
    if (!requireAccess()) return null;
    try {
      const sale = await completeSale({ items, paymentMethod, customerId, paymentProvider, customerName: contact.name || null, customerPhone: contact.phone || null, customerEmail: contact.email || null });
      await refresh();
      return { message: `Payment recorded. Invoice ${sale.invoice_number} is ready and the buyer contact was saved.`, invoiceNumber: sale.invoice_number, total: Number(sale.total) };
    } catch (caught) {
